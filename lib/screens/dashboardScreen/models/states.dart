class StatesModel {
  ...
  StatesModel({required this.total,required this.accepted,required this.cancelled,required this.completed, required this.depositAmount,
    required this.depositAmountPending,
    required this.balance,
    required this.disputed, required this.latestTransactions, required this.milestoneAmount, required this.noAccepted,
    required this.withdrawAmount, required this.withdrawAmountPending
  });
  static formJson(dynamic data){
    return StatesModel(balance: double.parse(data['balance']),
        total: int.tryParse(data['total'].toString()) ?? 0,
        accepted: int.tryParse(data['accepted'].toString()) ?? 0, cancelled:  int.tryParse(data['cancelled'].toString()) ?? 0,
        completed: int.tryParse(data['completed'].toString()) ?? 0,
    milestoneAmount: double.tryParse(data['milestoneAmount'].toString()) ?? 0,
        depositAmount: double.tryParse(data['depositAmount'].toString()) ?? 0,
        depositAmountPending: double.tryParse(data['depositAmountPending'].toString()) ?? 0,
        ...
        withdrawAmountPending: double.tryParse(data['withdrawAmountPending'].toString()) ?? 0
    );
  }
  updateData(dynamic data){
    ...
    depositAmountPending =  double.parse(data['depositAmountPending'].toString());
    withdrawAmountPending =  double.parse(data['withdrawAmountPending'].toString());
